...
import {
  buildBaseUrl, getNumberOfResults, resultShowedNumber, searchViator,
} from '../obtainArray.js';

describe('Test base parameters of obtainArray.js', () => {
  it('Should return base url', () => {
    for (const word in ['hola', 'chau']) {
      assert.equal(
        buildBaseUrl(word),
  ...
  it('Should return total number of results, given the result phrase', () => {
    for (const numberOfResults in [1, 23, 456, 7890, 98765]) {
  ...
  it('Should return number of shown results, given the result phrase', () => {
    for (const numberOfResults in [1, 23, 456, 7890, 98765, 432101]) {
  ...
  it('Should return an array of results like: {title, url}', async () => {
    const results = await searchViator('tango', 1, true);

...
export const buildBaseUrl = (searchTerm) =>
	`https://www.viator.com/es-ES/searchResults/all?text=${searchTerm}`;
...
export async function searchViator(searchTerm, maxProgramsToGet, isTest) {
	...
	async function showMoreResults() {
		...
		while (showing < maxProgramsToGet && showing < numberOfResults) {
			let resultPhrase;
			try {
				resultPhrase = await page.evaluate(
					() => document.querySelector('.title-count').textContent
				);
				await page
					.waitForSelector('.next-page-button')
					.then(async () => {
	...
	async function grabArray() {
		return page.evaluate((maxProgramsToGet) => {
			const hasMaxProgramsToGet = !!(
				typeof maxProgramsToGet === 'number' && maxProgramsToGet !== 0
			);

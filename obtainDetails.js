...
const puppeteerInit = async (URL) => {
	// ver comentario en linea 3
...
async function grabInitiallyShowedData() {
	...
	try {
		programData.duration = await page.evaluate(
			() =>
				document.querySelector(
					'[class^="productAttributesList"] > li > [class^="item"] > div:nth-child(2)'
				).textContent
		);
	} catch {}
	try {
		programData.overview = await page.evaluate(
			() => document.querySelector('[data-automation="product_overview"] > div > div').textContent
		);
	} catch {}
	try {
		programData.overviewTags = await page.evaluate(() =>
			Array.from(
				document.querySelectorAll(
					'div[data-automation="product_overview"] > ul[class^=featureList] > li > div'
				)
...
async function expandAndGrabSingleElement(buttonToClickSelector, grabDataFromSelector) {
	try {
		await page
			.waitForSelector(buttonToClickSelector)
			.then(await page.click(buttonToClickSelector))
			.then(await page.waitForSelector(grabDataFromSelector));
		return await page.evaluate(
			(grabDataFromSelector) => document.querySelector(grabDataFromSelector).textContent,
...
async function expandAndGrabLanguages() {
	// pensaba exportarla, pero la testeo dentro de grabData
...
async function expandAndGrabImageURLs() {
	// pensaba exportarla, pero la testeo dentro de grabData
	...
	try {
		...
		while (imgArrayLenght < 5 && tries < 10) {
			await page.click(buttonToClickSelector).then(await new Promise((r) => setTimeout(r, 500)));
			imgArrayLenght = await page.evaluate(
				(grabDataFromSelector) => document.querySelectorAll(grabDataFromSelector).length,
				grabDataFromSelector
			);
			tries += 1;
		}
		return await page.evaluate(
			(grabDataFromSelector) =>
				Array.from(document.querySelectorAll(grabDataFromSelector)).map((element) => element.src),
			grabDataFromSelector
		);
	} catch (error) {
		return [];
